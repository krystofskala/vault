...
import { emptyCustomPackContent, type CustomActionSpec, type CustomPoseSpec } from "../shimeji/customContent";
 ...
 * "Build your own") fails this check, and the caller warns before letting the random-options save
 * path silently replace that hand-authored logic. */
 ...
 * completely unmodified to cut each option's frames — the same composition CharacterWizardModal's
...
export class AnimationOptionsModal extends Modal {
	...
	constructor(
		...
		/** Called once, after a successful save or reset, so the wizard's checklist can refresh its
		 * own "N options" label without a manual reopen. */
	...
	private async save(): Promise<void> {
		...
		const packContent = this.plugin.settings.customContent[this.packId] ?? emptyCustomPackContent();
		const actions = [...packContent.actions.filter((a) => a.name.trim() !== this.actionName), spec];
		this.plugin.settings.customContent[this.packId] = { ...packContent, actions };
	...
	private async resetToStandard(): Promise<void> {
		const packContent = this.plugin.settings.customContent[this.packId];
		if (packContent) {
			const actions = packContent.actions.filter((a) => a.name.trim() !== this.actionName);
			this.plugin.settings.customContent[this.packId] = { ...packContent, actions };
			await this.plugin.saveSettings();
			this.plugin.applyCustomContent();
		}
