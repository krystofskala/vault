...
/**
 * Lets a character replace one action's whole animation with several alternatives ("options") —
 * e.g. two or three different Walk cycles cut from a richer game sprite sheet — instead of being
 * forced to fit the standard schema's fixed handful of pose slots. Built entirely as generated
 * CustomActionSpec data flowing through the existing CustomContentBuilder/mergeCustomContent
 * pipeline (see settings.customContent), so it needs no changes to ActionRunner or the expression
 * engine: ActionRunner.chooseAnimationVariant already re-evaluates an action's Animation
 * conditions fresh every single time the action starts (never cached), and Math.random() is
 * already a real, supported call inside a condition (RuntimeContext.ts's call()) — both verified
 * directly against source before writing this file, not assumed.
 */
